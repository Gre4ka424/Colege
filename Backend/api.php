...
function generateJWT($user_id, $role_id) {
    ...
    $payload = [
        'iat' => $issued_at,         // Token creation time
...
// GET /api.php/faculties - get all faculties
...
// GET /api.php/users - get all users
...
// POST /api.php/users - register a new user
if ($method === 'POST' && isset($request[0]) && $request[0] === 'users') {
    try {
        ...
        $role_id = isset($data['role_id']) ? $data['role_id'] : 3; // Default role 3 (pending)
...
// POST /api.php/login - authenticate user
if ($method === 'POST' && isset($request[0]) && $request[0] === 'login') {
    try {
        ...
        if ($user && password_verify($data['password'], $user['password_hash'])) {
            ...
            // Get full user data, but exclude password_hash
            unset($user['password_hash']);
            
            // Split full name into components (if needed)
...
// GET /api.php/achievements - get all achievements
...
// GET /api.php/profile - get current user profile (protected endpoint)
...
// GET /api.php/faculties/:id - get specific faculty by ID
if ($method === 'GET' && isset($request[0]) && $request[0] === 'faculties' && isset($request[1]) && is_numeric($request[1])) {
    try {
        ...
        if ($faculty) {
            // Ensure that faculty name is available both as faculty_name and name
            if (isset($faculty['name']) && !isset($faculty['faculty_name'])) {
                $faculty['faculty_name'] = $faculty['name'];
            } else if (isset($faculty['faculty_name']) && !isset($faculty['name'])) {
                $faculty['name'] = $faculty['faculty_name'];
...
// Protected endpoint: POST /api.php/achievements - add new achievement
